...
// Codigo Cliente
export default function Entrante ({ entrante }) {
  ...
  const addToCart = () => {
    addBocata({ ...entrante, price: entrante.price[Size], quantity: Quantity, size: Size });
...
export async function getStaticPaths() {
  const paths = await client.fetch(
    `*[_type == "entrante" && defined(slug.current)][].slug.current`
  );
  return {
...
export async function getStaticProps(context) {
  ...
  const entrante = await client.fetch(`
    *[_type == "entrante" && slug.current == '${slug}'][0]
  `);
  return {
    props: {
      entrante,
